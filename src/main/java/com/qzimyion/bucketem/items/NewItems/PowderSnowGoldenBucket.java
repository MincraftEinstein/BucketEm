...
public class PowderSnowGoldenBucket extends BlockItem implements FluidModificationItem {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        int fluidLevel = (int) stack.getOrDefault(ModDataComponents.FLUID_LEVEL, NbtComponent.DEFAULT);
        if (state.getBlock() instanceof PowderSnowBlock powderSnowBlock && player != null && !player.isSneaking() && fluidLevel < 2) {
            ...
            stack.getOrDefault(ModDataComponents.FLUID_LEVEL, fluidLevel + 1);
            if (!world.isClient) {
    ...
    public ItemStack getRecipeRemainder(ItemStack stack) {
        int level = (int) stack.getOrDefault(ModDataComponents.FLUID_LEVEL, NbtComponent.DEFAULT);
        if (level > 0) {
            ItemStack newStack = new ItemStack(ModItems.GOLDEN_POWDER_SNOW_BUCKET);
            newStack.getOrDefault(ModDataComponents.FLUID_LEVEL, level - 1);
            return newStack;
        }
