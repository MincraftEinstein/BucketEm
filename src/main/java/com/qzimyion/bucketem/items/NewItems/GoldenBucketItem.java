...
import static com.qzimyion.bucketem.items.ModDataComponents.*;
...
public class GoldenBucketItem extends Item implements FluidModificationItem {
    ...
    public static ItemStack resetFluidLevel(ItemStack stack) {
        stack.set(FLUID_LEVEL, 0);
        return stack;
    }

    public static void setFluidLevel(ItemStack stack, int level) {
        stack.set(FLUID_LEVEL, level);
    }

    public static void decreaseFluidLevel(ItemStack stack) {
        if (stack.contains(FLUID_LEVEL)){
            int level = stack.get(FLUID_LEVEL);
            if (level > 0) stack.set(FLUID_LEVEL, level -1);
        }
    }
    ...
    public static ItemStack increaseFluidLevel(ItemStack stack) {
        if (stack.contains(FLUID_LEVEL)){
            int level = stack.get(FLUID_LEVEL);
            if (level < 3) stack.set(FLUID_LEVEL, level + 1);
        }
        return stack;
    }

    public static boolean canBeFilled(ItemStack stack) {
        int count = stack.getOrDefault(FLUID_LEVEL, 0);
        return stack.set(FLUID_LEVEL, count) < 2;
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        int bucketLevel = stack.getOrDefault(FLUID_LEVEL, 0);
    ...
    public static ItemStack getEmptySuccessItem(ItemStack stack, PlayerEntity player) {
        int level = stack.getOrDefault(FLUID_LEVEL, 3);
